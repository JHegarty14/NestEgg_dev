import getTxtOption from './getTxtOption';

const toLower = (val = '') =>
  (val === null ? '' : val).toString().toLowerCase();

export default function matchTxtByIndex(value, options, allMatches) {
  ...
  const foundOptions = options.reduce((opts, opt, idx) => {
    ...
    const optValue =
      opt && opt.hasOwnProperty('value')
        ? opt.value
        : typeof opt === 'string' ? opt : null;
    const optText = getTxtOption(opt);
    const matchPosition = toLower(optText).indexOf(lowerText);

    if (
      (optValue === value && opt !== null) ||
      (optText &&
        lowerText &&
        (allMatches ? matchPosition !== -1 : matchPosition === 0))
    ) {
      return [
        ...opts,
        [idx, optText, optValue, matchPosition, optText.toLowerCase()]
      ];
    }
