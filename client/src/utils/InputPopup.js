...
class InputPopup extends Component {
    ...
    static defaultProps = {
      onRenderCaret: (styling, isActive, isHovered, children) => (
        <div {...styling('inputEnhancementsCaret', isActive, isHovered)}>
          {children}
        </div>
      ),
    ...
    renderCaretSVG(styling, isActive, hovered, popupShown) {
      const svgStyling = styling(
        'inputEnhancementsCaretSvg',
        isActive,
        hovered,
        popupShown
      );
    ...
    componentDidUpdate(prevProps, prevState) {
      ...
      if (
        prevState.popupShown !== this.state.popupShown &&
        this.props.onPopupShownChange
      ) {
    ...
    renderInput(styling, restProps) {
      ...
      return renderChildren(children, inputProps, customProps, registerInput);
    }
